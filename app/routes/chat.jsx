  ...
  try {
      ...
      finalMessage = await claudeService.streamConversation(
        ...
        },
        {
          ...
          onToolUse: async (content) => {
            ...
            // --- START: wellness keyword mapping ---
            const wellnessMap = {
              sleep: "Spermidine",
              insomnia: "Spermidine",

              immunity: "Urolithin A",
              immune: "Urolithin A",

              inflammation: "Liquid Turmeric",
              joint: "Liquid Turmeric",

              energy: "NMN + Resveratrol",
              fatigue: "NMN + Resveratrol",
              tired: "NMN + Resveratrol",

              longevity: "NMN + Resveratrol",
              aging: "NMN + Resveratrol",
            ...
            const userQuery = String(userMessage || "").toLowerCase();

            for (const key in wellnessMap) {
              if (userQuery.includes(key)) {
                mappedQuery = wellnessMap[key];
                break;
              }
            }

            if (  mappedQuery && toolName === "search_catalog"   ) {
              toolArgs.query = mappedQuery;             
            }
            // --- END mapping ---
    ...
    if (productsToDisplay.length > 0) {
      ...
      for (const product of productsToDisplay) {
        ...
        const detailsData = JSON.parse(details.content[0].text);

        const productDetails = detailsData.product;

        const subscriptionPlans = productDetails.selling_plan_groups || [];
        const subscriptions = subscriptionPlans.map(group => {
            const plan =
            ...
            return {
              ...
              name: plan?.name,
              discount:
                plan?.price_adjustments?.[0]?.value || 0
            };

          });

        product.subscriptionPlans =  subscriptions;
        product.subscriptionAvailable = subscriptions.length > 0;
