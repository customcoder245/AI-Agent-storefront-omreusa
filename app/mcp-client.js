...
class MCPClient {
   ...
   */
  async callStorefrontToolbkp(toolName, toolArgs) {
    try {
      console.log(
      ...
      // Only apply query normalization for catalog searches
      if (
        toolName === "search_catalog" &&  toolArgs?.query ) {
        const originalQuery =
          toolArgs.query.toLowerCase().trim();
        ...
        // Normalize broad wellness searches
        let normalizedQuery = toolArgs.query;

        if (
          originalQuery.includes("longevity") ||
          originalQuery.includes("healthy aging") ||
          originalQuery.includes("anti aging") ||
          originalQuery.includes("cellular health")
        ) {
          normalizedQuery = "longevity";
        }
        else if (
          originalQuery.includes("energy") ||
          originalQuery.includes("fatigue")
        ) {
          normalizedQuery = "energy";
        }
        else if (
          originalQuery.includes("focus") ||
          originalQuery.includes("brain health") ||
          originalQuery.includes("mental clarity")
        ) {
          normalizedQuery = "focus";
        }
        else if (
          originalQuery.includes("recovery")
        ) {
          normalizedQuery = "recovery";
        }
        else if (
          originalQuery.includes("metabolism")
        ) {
          normalizedQuery = "metabolism";
        }
        else if (
          originalQuery.includes("inflammation")
        ) {
          normalizedQuery = "inflammation";
        }

        if (normalizedQuery !== toolArgs.query) {
          console.log(
            `Mapped query "${toolArgs.query}" → "${normalizedQuery}"`
          );
        }

        // Shopify MCP expects catalog.query
        // toolArgs = {
        //   catalog: {
        ...
        toolArgs = {
            catalog: {
              query: normalizedQuery,
              // Include additional fields to search
  ...
  async callStorefrontTool(toolName, toolArgs) {
    try {
      ...
      console.log("Calling storefront tool:", toolName, JSON.stringify(toolArgs, null, 2));

      if (toolName === "search_catalog" && toolArgs?.query) {
        // Just pass the query as-is — the prompt now handles the mapping correctly
        toolArgs = {
          catalog: {
      ...
      console.log("Storefront tool response:", toolName, JSON.stringify(response, null, 2));

      if (response?.result?.isError) {  console.error(  "MCP TOOL ERROR:",   response.result.content ); }
