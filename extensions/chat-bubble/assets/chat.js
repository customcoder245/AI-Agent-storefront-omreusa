...
(function() {
  ...
  const ShopAIChat = {
    ...
    Message: {
      ...
      add: function(text, sender, messagesContainer) {
        ...
        ShopAIChat.UI.scrollToBottom(true);
    ...
    API: {
      ...
      streamResponse: async function(userMessage, conversationId, messagesContainer) {
        ...
        try {
          ...
          // const streamUrl = 'https://ai-agent-storefront.onrender.com/chat';
          ...
          // Short delay to make AI feel like it's "thinking"
          await new Promise(resolve => setTimeout(resolve, 1200)); // 1.2 seconds
          console.log("Confirming if its working");
      ...
      handleStreamEvent: function(data, currentMessageElement, messagesContainer, userMessage, updateCurrentElement) {
        console.log(" Product handler:", JSON.stringify(data, null, 2));
        ...
        switch (data.type) {
          ...
          case 'chunk':
            ...
            if (!currentMessageElement) {
              ...
              currentMessageElement.classList.add(
                'shop-ai-message',
                'assistant'
              );
          ...
          case 'new_message':
            ...
            // only create next message when text arrives
  ...
  break;
      ...
      fetchChatHistory: async function(conversationId, messagesContainer) {
        try {
          ...
          const historyUrl = `https://ai-agent-storefront.onrender.com/chat?history=true&conversation_id=${encodeURIComponent(conversationId)}`;
          ...
          const welcomeMessage = window.shopChatConfig?.welcomeMessage || "👋 Hi, I'm here to help you age well. Tell me your health goals and I'll point you to the right Omre supplement — or ask me anything about our ingredients, science, shipping, or returns.";
          ShopAIChat.Message.add(welcomeMessage, 'assistant', messagesContainer);
    ...
    Auth: {
      ...
      startTokenPolling: function(conversationId, messagesContainer) {
        ...
        const poll = async () => {
          ...
          try {
            const tokenUrl = 'https://ai-agent-storefront.onrender.com/auth/token-status?conversation_id=' +
  ...
  let addButton = null;
  let currentVariantId = selectedVariant?.id?.split('/').pop();
  let currentSellingPlanId = product.subscriptionPlans?.[0]?.id?.split('/').pop();
  let purchaseType = 'onetime'; // default

  // ======================
  // Map variant -> subscription plan
  // ======================
  const variantPlanMap = {};
  product.variants.forEach((variant, i) => {
    if (product.subscriptionPlans?.[i]) {
      variantPlanMap[variant.id] = product.subscriptionPlans[i];
    }
  });
  ...
  if (product.tags?.length) {
    ...
    tags.classList.add('shop-ai-product-description');
    tags.innerHTML = product.tags
      .map(tag => tag.replace(/\b\w/g, l => l.toUpperCase()))
      .join(' • ');
  ...
  if (product.variants?.length > 1) {
    ...
    variantSelect.classList.add('shop-ai-variant-select');
    ...
    variantSelect.addEventListener('change', function() {
      selectedVariant = product.variants.find(v => v.id === this.value);
      currentVariantId = selectedVariant?.id?.split('/').pop();

      if (selectedVariant?.media?.[0]?.url) image.src = selectedVariant.media[0].url;

      // Update subscription plan for selected variant
      if (purchaseType === 'subscription') {
        const plan = variantPlanMap[selectedVariant.id];
        currentSellingPlanId = plan?.id?.split('/').pop() || null;
        saveLabel.textContent = plan ? `Save ${plan.discount}%` : '';
      }
  ...
  saveLabel.classList.add('shop-ai-subscription-save');

  if (product.subscriptionPlans?.length) {
    const purchaseContainer = document.createElement('div');
    purchaseContainer.classList.add('shop-ai-purchase-container');
    purchaseContainer.innerHTML = `
      <label class="shop-ai-purchase-option">
        <input type="radio" name="purchase-${product.id}" value="onetime" checked>
        One Time Purchase
      </label>
      <label class="shop-ai-purchase-option">
        <input type="radio" name="purchase-${product.id}" value="subscription">
        Subscribe & Save
      </label>
    `;
    info.appendChild(purchaseContainer);
    info.appendChild(saveLabel);

    purchaseContainer.querySelectorAll('input[type="radio"]').forEach(radio => {
  ...
  addButton.classList.add('shop-ai-add-to-cart');
  actions.appendChild(addButton);

  function updatePrice() {
    let amount = selectedVariant?.price?.amount ? selectedVariant.price.amount / 100 : 0;
    if (purchaseType === 'subscription' && currentSellingPlanId) {
      const selectedPlan = product.subscriptionPlans.find(p => p.id.endsWith(currentSellingPlanId));
      if (selectedPlan) amount = amount - (amount * selectedPlan.discount / 100);
    }
    price.textContent = `${selectedVariant.price.currency} ${amount.toFixed(2)}`;
  }
  ...
  function updateCartButtonText() {
    ...
    addButton.textContent = `Add To Cart - $${amount.toFixed(2)}`;
  }
  ...
  addButton.addEventListener('click', async function(e) {
    ...
    if (purchaseType === 'subscription' && currentSellingPlanId) {
      payload.selling_plan = Number(currentSellingPlanId);
    }
